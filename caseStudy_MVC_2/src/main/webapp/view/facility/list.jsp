...
<html>
<head>
    <title>Show list facility</title>
    ...
    <table id="facilityTable" class="table table-light table-striped table-bordered">
        ...
        <c:forEach varStatus="status" var="facility" items="${facilityList}">
            <tr>
                ...
                <td>₫${String.format("%.0f", facility.getCost())}</td>
                <td class="text-center">${facility.getMaxPeople()}</td>

                <c:if test="${facility.getStandardRoom()!=null}">
                ...
                <c:if test="${facility.getNumberOfFloors()!=0}">
                    <td class="text-center">${facility.getNumberOfFloors()}</td>
                </c:if>
                <c:if test="${facility.getNumberOfFloors()==0}">
                ...
                <c:forEach var="facilityType" items="${facilityTypes}">
                    <c:if test="${facilityType.getFacilityTypeId() == facility. getFacilityTypeId() }">
                        <td>${facilityType.getFacilityTypeName()}</td>
                    </c:if>
                </c:forEach>
